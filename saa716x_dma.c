 ...
 *  structure.
 */
int saa716x_allocate_ptable(struct saa716x_dmabuf *dmabuf)
{
	...
	dprintk(SAA716x_DEBUG, 1, "Allocated at 0x%llx (phys)", dmabuf->mem_ptab_phys);

	return 0;
}
EXPORT_SYMBOL_GPL(saa716x_allocate_ptable);


void saa716x_free_ptable(struct saa716x_dmabuf *dmabuf)
{
	struct saa716x_dev *saa716x	= dmabuf->saa716x;
	struct pci_dev *pdev		= saa716x->pdev;
...
EXPORT_SYMBOL_GPL(saa716x_free_ptable);
...
void saa716x_dmabuf_sgpagefill(struct saa716x_dmabuf *dmabuf, struct scatterlist *sg_list, int nents, int offset)
{
	struct saa716x_dev *saa716x	= dmabuf->saa716x;
	struct pci_dev *pdev		= saa716x->pdev;
	...
	/* create page table */
	for (i = 0; i < nents; i++) {
		//dprintk(SAA716x_DEBUG, 1, "sg_cur->length = 0x%x, sg_cur->offset = 0x%x", sg_cur->length, sg_cur->offset);
		...
		for (j = 0; (j * SAA716x_PAGE_SIZE) < sg_cur->dma_length; j++) {
			
			if ((offset) >= SAA716x_PAGE_SIZE) {
				offset -= SAA716x_PAGE_SIZE;
				continue;
			}
			addr = ((u64)sg_dma_address(sg_cur)) + (j * SAA716x_PAGE_SIZE);
			BUG_ON(addr == 0);
...
EXPORT_SYMBOL_GPL(saa716x_dmabuf_sgpagefill);
