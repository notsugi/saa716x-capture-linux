...
static const u32 msi_int_tagack[] = {
	MSI_INT_TAGACK_VI0_0 | MSI_INT_TAGACK_VI0_1 | MSI_INT_TAGACK_VI0_2,
	MSI_INT_TAGACK_VI1_0 | MSI_INT_TAGACK_VI1_1 | MSI_INT_TAGACK_VI1_2
};
				 ...
				 struct vip_stream_params *stream_params)
{
	...
	start_line = stream_params->offset_y;
	end_line = 0;
	...
	if(stream_params->source_format == VIP_FMT_DEFAULT) {
		vin_format |= 2;
	} else {
		/* to receive TDA19977 output */
		vin_format |= 1;
	}
	SAA716x_EPWR(vi_port, VIN_FORMAT, vin_format);
	printk("%s: [VI%d] VIN_FORMAT=0x%x", __func__, port, vin_format);
	...
	if(stream_params->source_format == VIP_FMT_DEFAULT) {
		win_xystart = (start_x << 16) + (start_line + 1);
		win_xyend 	= ((start_x + stream_params->samples - 1) << 16) + end_line;
	} else {
		win_xystart = (start_x << 16) + start_line;
		win_xyend	= ((start_x + stream_params->samples) << 16) + end_line;
	}
	
	SAA716x_EPWR(vi_port, WIN_XYSTART, win_xystart);
	SAA716x_EPWR(vi_port, WIN_XYEND, win_xyend);
	printk("%s: [VI%d] WIN_XYSTART=0x%x", __func__, port, win_xystart);
	printk("%s: [VI%d] WIN_XYEND=0x%x", __func__, port, win_xyend);
	...
	printk("%s: [VI%d] PSU_WINDOW=0x%x", __func__, port, (stream_params->samples << 16) + num_lines);
		      ...
		      struct vip_stream_params *stream_params)
{
	...
	printk("%s: [VI%d] VI_MODE=0x%x", __func__, port, val);
		     ...
		     void (*worker)(unsigned long))
{
	...
	printk("%s: VI%d initialized", __func__, port);

	return 0;
}
EXPORT_SYMBOL_GPL(saa716x_vip_init);

int saa716x_vip_init2(struct saa716x_dev *saa716x, int port,
		     void (*worker)(unsigned long))
{
	int n, i;
	int ret;
	...
	for (n = 0; n < 2; n++) {	
		saa716x->vip[port].dma_channel[n] = port * 3 + n;
		for (i = 0; i < VIP_BUFFERS; i++) {
			saa716x->vip[port].dma_buf[n][i].saa716x = saa716x;
			ret = saa716x_allocate_ptable(&saa716x->vip[port].dma_buf[n][i]);
			if (ret)
				return ret;
		}
	}
...
EXPORT_SYMBOL_GPL(saa716x_vip_init2);
...
int saa716x_vip_exit2(struct saa716x_dev *saa716x, int port)
{
	int n;
	int i;
