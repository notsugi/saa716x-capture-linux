...
int saa716x_check_eeprom(struct saa716x_dev *saa716x)
{
	struct saa716x_romhdr rom_header;
	u8 buf[DUMP_BYTES];
	int i, err = 0;
...
EXPORT_SYMBOL_GPL(saa716x_check_eeprom);
...
int saa716x_parse_eeprom(struct saa716x_dev *saa716x)
{
	struct saa716x_romhdr rom_header;
	struct saa716x_devinfo *device;
	...
	if (ret < 0) {
		printk("%s: EEPROM read error <%d>", __func__, ret);
		goto err0;
	}
	...
	if (ret != 0) {
		printk("%s: header read failed <%d>", __func__, ret);
		goto err0;
	}
	...
	device = kzalloc(sizeof (struct saa716x_devinfo) * rom_header.devices,
			GFP_KERNEL);
	if (device == NULL) {
		printk("%s: out of memory (device)", __func__);
		goto err0;
	}

	for (i = 0; i < rom_header.devices; i++) {
		dprintk(SAA716x_DEBUG, 1,
			"SAA%02x ROM: ===== Device %d at offset 0x%02x =====\n",
			saa716x->pdev->device, i, offset);
...
EXPORT_SYMBOL_GPL(saa716x_parse_eeprom);
